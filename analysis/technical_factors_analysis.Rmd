...
p_comps <- 1:6
info <- sampleinfo %>% 
     select(c(Individual, Sex, Replicate, Passage_MSC, treatment, RIN, LibraryPrepBatch)) #subset sample info for technical/biological variables
...
```{r RUV}
#The RUVSeq vignette loads raw counts and uses the RUVSeq package to filter and normalize data (upper quantile normalization) before performing RUVs. Here, I just load in the pre-filtered/normalized cpm data.
...
set <- newSeqExpressionSet(as.matrix(filt_cpm),
                           phenoData = data.frame(sampleinfo, row.names=colnames(filt_cpm)))
set

set1 <- RUVs(set, rownames(filt_cpm), k=1, replicates)
pData(set1)
