...
plot_gene <- function(v, g) {
  ...
  single_gene_long <- separate(single_gene_long, col = "sample", sep = "_",
                                into = c("Individual", "Replicate", "Treatment"))
  single_gene_long <- single_gene_long %>% 
       mutate(ind_treatment = paste0(Individual, "_", Treatment))
  single_gene_long$ind_treatment <- factor(single_gene_long$ind_treatment, levels = c("18855_U", "18856_U", "19160_U", "18855_S", "18856_S", "19160_S"))

  return(ggplot(single_gene_long, aes(x = ind_treatment, y = log2cpm, fill = ind_treatment)) +
    geom_boxplot() +
    labs(title = g, x = "Treatment", y = expression("Expression level (" * log[2] * " cpm)"))
  )
}
