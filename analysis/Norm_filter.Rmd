...
# UQ normalization for full dataset including sample 2

```{r}
upperquartile_full <- calcNormFactors(raw_counts_full, method = "upperquartile")
upperquartile_full <- cpm(upperquartile_full, log=TRUE, normalized.lib.sizes = T)

cutoff <- 2.5

keep <- rowSums( upperquartile_full > cutoff ) >=4

counts_upperquartile_full <- raw_counts[keep,]
filtered_upperquartile_full <- upperquartile_full[keep,]
dim(filtered_upperquartile_full)

saveRDS(counts_upperquartile_full, "data/filtered_counts_full.rds")
saveRDS(filtered_upperquartile_full, "data/norm_filtered_counts_full.rds")
...
# Perform PCA
pca_genes <- prcomp(t(filtered_upperquartile_full), scale = T)
scores <- pca_genes$x
...
### PCA norm+filt Data
for (n in 1:5){
  col.v <- pal[as.integer(sampleinfo_full$Individual)]
