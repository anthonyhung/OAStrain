...
counts <- as.matrix(Combined.common.allGenes@assays$RNA@counts)
labels <- sapply(strsplit(Combined.common.allGenes@meta.data$Cell.Type,"_"), `[`, 1)
labels <- sapply(strsplit(labels," "), `[`, 1)


# labels for "known" clusters
index_hep <- which(labels=="Hepatocyte")
index_Chondro <- which(labels=="Chondro")
index_iPSC <- which(labels=="iPSC")

known_samples <- c(index_hep, index_Chondro, index_iPSC)

class_labs <- c(rep("Hepatocyte", length(index_hep)),
                rep("Chondrocyte", length(index_Chondro)),
                rep("iPSC", length(index_iPSC))
                )



# Perform topic modeling k=4
Topic_clus_4 <- classtpx::class_topics(
    t(counts),
    K=4,
    ...
    method="omega.fix",
    tol=0.01)
save(Topic_clus_4, file="data/merged_topic_fit_4_classtpx_omega_fix_scRNA.rda")
Topic_clus_4 <- get(load(file="data/merged_topic_fit_4_classtpx_omega_fix_scRNA.rda"))

omega <- Topic_clus_4$omega
annotation <- data.frame(
...
Topic_clus_4 <- classtpx::class_topics(
    ...
    method="omega.fix",
    tol=0.01)
save(Topic_clus_5, file="data/merged_topic_fit_5_classtpx_omega_fix_scRNA.rda")
Topic_clus_5 <- get(load(file="data/merged_topic_fit_5_classtpx_omega_fix_scRNA.rda"))

omega <- Topic_clus_5$omega
annotation <- data.frame(
  sample_id = paste0("X", c(1:NROW(omega))),
  tissue_label = as.factor(labels))
rownames(omega) <- annotation$sample_id
...
library(fastTopics)
counts_allgenes <- Combined.common.allGenes@assays$RNA@counts
fit1 <- fit_poisson_nmf(counts_allgenes,k = 3,numiter = 500,method = "mu")
