...
# Additional RUVseq analysis: Using top 100 LEAST variable genes as control genes to get an output of W variables

```{r identify least variable genes}
RLE <- calcNormFactors(counts_upperquartile, method = "RLE")
RLE <- cpm(RLE, log=FALSE, normalized.lib.sizes = T)

#compute CV (stdev/mean) and rank from least to most; pick 100 least variable
cv <- apply(RLE, 1, function(x) sd(x)/mean(x))
least_var_genes <- names(head(sort(cv), 100))



# upperquartile <- calcNormFactors(counts_upperquartile, method = "upperquartile")
# upperquartile <- cpm(upperquartile, log=TRUE, normalized.lib.sizes = T)
# 
# #compute CV (stdev/mean) and rank from least to most; pick 100 least variable
# cv <- apply(upperquartile, 1, function(x) sd(x)/mean(abs(x)))
# least_var_genes <- names(head(sort(cv), 100))
```
...
varPart <- fitExtractVarPartModel( filtered_RLE, form, sample_info )
...
#no interaction
varPart <- fitExtractVarPartModel( filtered_RLE, form_no_interaction, sample_info )
vp <- sortCols(varPart)
plotVarPart( vp )
