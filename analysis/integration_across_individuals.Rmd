...
features.plot.chond <- c("TIMP3", "TIMP2", "COL11A1", 
                   "SOX5", "SOX6",
                   "TGFBI")
features.plot.msc <- c("THY1",  "NT5E") #cd44 is a negative marker (looks like )
...
```{r run lmm}
form <- ~ 0 + seurat_clusters + (1|individual) + n_cells
...
SCTransformed_data <- SCT.integrated@assays$SCT@data
cluster_membership <- SCT.integrated@meta.data$integrated_snn_res.0.4

clust0 <- SCTransformed_data[,cluster_membership == 0]
clust1 <- SCTransformed_data[,cluster_membership == 1]
clust2 <- SCTransformed_data[,cluster_membership == 2]

t.test.results <- matrix(NA, nrow = nrow(SCTransformed_data), ncol = 6)
colnames(t.test.results) <- c("0vs1 p", "0vs1 T.stat", "0vs2 p", "0vs2 T.stat", "1vs2 p", "1vs2 T.stat")
rownames(t.test.results) <- rownames(SCTransformed_data)

for(gene in 1:nrow(SCTransformed_data)){
     t.test.results[gene,1] <- t.test(x = clust0[gene,], y = clust1[gene,], alternative = "two.sided", var.equal = FALSE)$p.value
     t.test.results[gene,2] <- t.test(x = clust0[gene,], y = clust1[gene,], alternative = "two.sided", var.equal = FALSE)$statistic
     t.test.results[gene,3] <- t.test(x = clust0[gene,], y = clust2[gene,], alternative = "two.sided", var.equal = FALSE)$p.value
     t.test.results[gene,4] <- t.test(x = clust0[gene,], y = clust2[gene,], alternative = "two.sided", var.equal = FALSE)$statistic
     t.test.results[gene,5] <- t.test(x = clust1[gene,], y = clust2[gene,], alternative = "two.sided", var.equal = FALSE)$p.value
     t.test.results[gene,6] <- t.test(x = clust1[gene,], y = clust2[gene,], alternative = "two.sided", var.equal = FALSE)$statistic
}
```
