...
n_sorted <- n_sorted %>% 
     mutate(SES = if_else(V7 < 0, true = qnorm(V6/2)/sqrt(86), false = qnorm(1-V6/2)/sqrt(86-2)))
...
```{r rhinovirus}
rhinovirus_data <- as.tibble(read.table("data/caliskan_etal/eQTL_results_cis_response.txt", 
                              sep = "\t",
                              header = TRUE))

#convert t.stat into the SES by adjusting for the sample size (sqrt(98-2))
rhinovirus_data <- rhinovirus_data %>% 
     mutate(SES = t.stat/sqrt(98-2))

#add column to denote which condition the association comes from
rhinovirus_data <- rhinovirus_data %>% 
...
rhinovirus_data_r_prelim <- rhinovirus_data_r %>% 
     mutate(SES_prelim = if_else(beta < 0, true = qnorm(p.value/2)/sqrt(98), false = qnorm(1-p.value/2)/sqrt(86)))
