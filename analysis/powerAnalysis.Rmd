...
# For Natalia (for R01 Grant)

```{r power curve for R01}
p <- ggplot(data.frame(x = c(0, 2.5)), aes(x = x)) +
     stat_function(fun = power_function, args = list(n = 6),
                      aes(colour = "6"), size = 1.5) +
     stat_function(fun = power_function, args = list(n = 10),
                      aes(colour = "10"), size = 1.5) +
     stat_function(fun = power_function, args = list(n = 30),
                      aes(colour = "30"), size = 1.5) +
     stat_function(fun = power_function, args = list(n = 70),
                      aes(colour = "70"), size = 1.5) +
     scale_x_continuous(name = "Standardized Effect Size",
                              limits=c(0, 2.5)) +
     scale_y_continuous(name = "Power",
                           limits = c(0,1)) +
     ggtitle("eQTL Power Curves") +
     scale_colour_manual("Sample Size", breaks = c("6", "10", "30", "70"), values = colors) +
     theme_bw() +
     geom_hline(yintercept = .8, linetype = "dashed", color = "red")
p

#figure out where 70 crosses dotted line
n70_ses_threshold <- (qnorm(.8) - qnorm(alpha/2))/sqrt(100)
n70_ses_threshold
```
