...
# saveRDS(data, "output/mash_full_data_INT.rds")
...
m_INT_1 <- readRDS("output/mash_all_refit_500k_INT_1.rds")
m_INT_2 <- readRDS("output/mash_all_refit_500k_INT_2.rds")
m_INT_3 <- readRDS("output/mash_all_refit_500k_INT_3.rds")
m_INT_4 <- readRDS("output/mash_all_refit_500k_INT_4.rds")
m_INT_5 <- readRDS("output/mash_all_refit_500k_INT_5.rds")
m_INT_6 <- readRDS("output/mash_all_refit_500k_INT_6.rds")
m_INT_7 <- readRDS("output/mash_all_refit_500k_INT_7.rds")
m_INT_8 <- readRDS("output/mash_all_refit_500k_INT_8.rds")

m_1 <- readRDS("output/mash_all_refit_500k_1.rds")
m_2 <- readRDS("output/mash_all_refit_500k_2.rds")
m_3 <- readRDS("output/mash_all_refit_500k_3.rds")
m_4 <- readRDS("output/mash_all_refit_500k_4.rds")
m_5 <- readRDS("output/mash_all_refit_500k_5.rds")
m_6 <- readRDS("output/mash_all_refit_500k_6.rds")
m_7 <- readRDS("output/mash_all_refit_500k_7.rds")
m_8 <- readRDS("output/mash_all_refit_500k_8.rds")
...
Afile <- gzfile('/project2/gilad/mcward/eqtls/A-adjust.txt.gz','rt')
Bfile <- gzfile('/project2/gilad/mcward/eqtls/B-adjust.txt.gz','rt')
Cfile <- gzfile('/project2/gilad/mcward/eqtls/C-adjust.txt.gz','rt')
Dfile <- gzfile('/project2/gilad/mcward/eqtls/D-adjust.txt.gz','rt')
conditionA <- as_tibble(read.table(Afile, fill = TRUE, stringsAsFactors = FALSE))
conditionB <- as_tibble(read.table(Bfile, fill = TRUE, stringsAsFactors = FALSE))
conditionC <- as_tibble(read.table(Cfile, fill = TRUE, stringsAsFactors = FALSE))
conditionD <- as_tibble(read.table(Dfile, fill = TRUE, stringsAsFactors = FALSE))
...
get_effect_size <- function(df, index){
     ...
     return(sign(v['ALPHA'] - v['BETA']))
}

# find SNP-gene pairs that were tested in all conditions
common <- sort(Reduce(intersect, list(paste0(conditionA$TEST.SNP.ID, " ", conditionA$PHENO),
                           paste0(conditionB$TEST.SNP.ID, " ", conditionB$PHENO),
                           paste0(conditionC$TEST.SNP.ID, " ", conditionC$PHENO),
                           paste0(conditionD$TEST.SNP.ID, " ", conditionD$PHENO)
                           )))
length(common)

conditionA <- conditionA %>% 
     mutate(temp = paste0(TEST.SNP.ID, " ", PHENO)) %>% 
     filter(temp %in% common) %>% 
     dplyr::select(-temp)
