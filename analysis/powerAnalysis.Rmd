...
SNP_file_filtered <- SNP_file %>% 
     dplyr::filter(NA19160 != NA18855 | NA19160 != NA18856 | NA18856 != NA18855) %>% 
     dplyr::filter(!is.na(NA19160) & !is.na(NA18856) & !is.na(NA18855))
...
p <- ggplot(data.frame(x = c(0, 2)), aes(x = x)) +
     ...
     stat_function(fun = FPR_function, args = list(n = 100),
                      aes(colour = "100", linetype = "csQTL FPR"), size = 1, alpha = 0.9) +
...
# rhinovirus_f_ashr_out <- ash(betahat = rhinovirus_data_f$beta, sebetahat = rhinovirus_data_f$se)
...
# rhinovirus_r_ashr_out <- ash(betahat = rhinovirus_data_r$beta, sebetahat = rhinovirus_data_r$se)
...
# After (2), I suggest an alternative that does not depend on the number of genes or tests per gene, and more directly answers the question "what power would be achieved to detect a effect size that is plausible to see in real data, given the sample size?": recover the 50th, 90th, 95th, 99th, (etc.) percentiles of the fitted_g (this can be approximately done by evaluating ash_cdf on a dense grid), and report the power for those true effect sizes.
grid_of_effects <- seq(-1.5, 1.5, length.out = 1e3)
vector_of_percentiles <- cdf.ash(rhinovirus_f_ashr_out, grid_of_effects)
#look for the effect sizes that correspond to percentiles of interest

#add column for SES (based on ash output)
rhinovirus_data$SES <- rhinovirus_ashr_out$result$PosteriorMean/rhinovirus_ashr_out$result$PosteriorSD
