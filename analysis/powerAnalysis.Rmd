...
n_egenes <- matrix(NA, nrow = 4, ncol = 3)
colnames(n_egenes) <- c("n=58", "n=30", "n=10")
rownames(n_egenes) <- c("conditionA", "conditionB", "conditionC", "conditionD")
i <- 1
for(condition in list(conditionA, conditionB, conditionC, conditionD)){
     ...
     for(threshold in c(n58_ses_threshold, n30_ses_threshold, n10_ses_threshold)){
         n_egenes[i,j] <- length(unique(condition %>% filter(zscore/sqrt(15) > threshold | zscore/sqrt(15) < -threshold) %>% pull(PHENO)))
         j <- j + 1
     }
     i <- i + 1
}
