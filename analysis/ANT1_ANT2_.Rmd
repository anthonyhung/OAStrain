...
ANT1_seurat <- subset(ANT1_seurat, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)
ANT2_seurat <- subset(ANT2_seurat, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)
```
...
```{r PCA}
#ANT1_seurat <- RunPCA(ANT1_seurat, features = VariableFeatures(object = ANT1_seurat))
ANT2_seurat <- RunPCA(ANT2_seurat, features = VariableFeatures(object = ANT2_seurat))



# Only ANT2 works here, since only 17 cells passed QC in ANT1
...
DimPlot(ANT2_seurat, reduction = "umap")
