...
params:
  sample: "/project2/gilad/anthonyhung/Projects/OAStrain_project/YriMultiplex/YG-AH-2S-ANT-1_S1_L008/"
---
...
branks <- barcodeRanks(samp.raw, lower=10, fit.bounds=NULL)

#get only barcodes with rank above 100,000
branks<- branks[which(branks$rank <= 100000),]

#redefine samp.raw, subsetting to only the columns corresponding to the highest ranked 100,000 barcodes.in test, this left me with samp.raw having 94,209 columns. Why fewer? ties in barcode ranks?
...
#redefine samp.raw, subset to only columns/barcodes that are likely to represent drops with a cell. In test, only 9,103 barcodes left. This seems correct as we expect to capture ~10,000 cells from a 10x lane
samp.raw<- samp.raw[,which(samp.empty$FDR<=0.05)]
```

At this point, kenneth conversts rownames and assigns species in the HumanChimpEB data

Make a seurat object

Running this gives a message about non-unique features in the rownames(duplicates of gene names) and makes them unique. Thats why kenneth converts the rownames. I'll still leave this out for now, see if it becomes a problem
```{r}
samp.obj<- CreateSeuratObject(samp.raw, project="7week")
```
...
Notably, in the tutorial, they filter cells that have unique feature counts over 2,500 or less than 200 and filter cells that have over 5% MT counts. We (Kenneth and I) have not been filtering Human Chimp EBs for MT, and I don't believe we are filtering based solely on unique features(instead, we are using emptyDrops)
...
```
 18858 has higher gene count, and higher RNA count in a bunch of cells. Are these doublets from the same individual (maybe 18858 not fully dissociated?)

```{r, eval=F}
#Subsetting objects for each individual, this ended up not being necessary
FiveEleven.obj<- subset(samp.obj, subset = individual == 'SNG-NA18511')
OneSixty.obj<- subset(samp.obj, subset = individual == 'SNG-NA19160')
EightFiveEight<- subset(samp.obj, subset = individual == 'SNG-NA18858')
```
